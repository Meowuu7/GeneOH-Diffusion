...
def calculate_penetration_depth(subj_seq, tot_obj_verts, obj_faces):
...
if __name__=='__main__':
  ...
  ### TODO: fill those blanks ###
  obj_verts_sv_fn = "/data1/sim/mdm/tmp_data/case5/obj_verts_ws_60.npy"
  tot_obj_verts = np.load(obj_verts_sv_fn, allow_pickle=True)
  print(f"Object vertices loaded from {obj_verts_sv_fn} with shape {tot_obj_verts.shape}")
  single_obj_sv_fn = "/data1/sim/mdm/tmp_data/case5/object_0.obj"
  single_obj_verts, single_obj_faces = load_ply_data(single_obj_sv_fn)
  obj_faces = single_obj_faces
  
  
  sv_predicted_infos_fn_to_statistics = {}
  # st_idx = 58
  # ed_idx = 111
  # st_idx = 0
  # ed_idx = 246
  ...
  # for test_seq_idx in range(36, 37, 1):
    ...
    sv_predicted_info_fn = f"predicted_infos_seq_{test_seq_idx}_seed_{seed}_tag_{test_tag}.npy"
    sv_predicted_info_fn = f"predicted_infos_seq_300_seed_{cur_seed}_tag_rep_res_jts_hoi4d_scissors_t_300_.npy"
    ...
    test_tag = "rep_res_jts_hoi4d_scissors_t_300_"
    
    sv_predicted_info_fn = f"predicted_infos_seq_300_seed_{cur_seed}_tag_rep_res_jts_hoi4d_scissors_t_300_.npy"
    sv_predicted_info_fn_ours =  f"predicted_infos_seq_300_seed_{cur_seed}_tag_rep_res_jts_hoi4d_scissors_t_300_.npy"
    ...
    if os.path.exists(sv_predicted_info_fn) and os.path.exists(predicted_ours_info_fn): #  and os.path.exists(sv_optimized_info_fn):
      ...
      gt_joints = np.matmul(gt_joints, tot_obj_rot) + tot_obj_transl.reshape(tot_obj_transl.shape[0], 1, tot_obj_transl.shape[1]) #
      ...
      # gt_joints = data['tot_gt_rhand_joints'][:ws] # [0]
      ### for toch ###
      ...
      print(f"gt_joints: {gt_joints.shape}")
      smoothness = calculate_joint_smoothness(outputs)
      APD = calculate_penetration_depth(outputs, tot_obj_verts, obj_faces)
      dist_dist_avg = calculate_proximity_dist(outputs, gt_joints, tot_obj_verts, obj_faces)
      ...
      #### optimized_sv_dict ####
      cur_sv_dict = {
        'tot_obj_verts': tot_obj_verts,
        'joints': outputs,
        "obj_faces": obj_faces,
      }
      cur_sv_dict_fn = f"/data1/sim/mdm/tmp_data/case5/obj_joints_sv_seed_{cur_seed}.npy"
      np.save(cur_sv_dict_fn, cur_sv_dict)
      print(f"sv_dict saved to {cur_sv_dict_fn}")
      
      
      ### get joints_trans and base_pts_trans ### ## joints_trans, base_pts_trans 
      # joints_trans = np.matmul(outputs, tot_obj_rot) + tot_obj_transl.reshape(tot_obj_transl.shape[0], 1, tot_obj_transl.shape[1]) #
      
      joints_trans = outputs
      base_pts_trans = np.matmul(tot_base_pts, tot_obj_rot) + tot_obj_transl.reshape(tot_obj_transl.shape[0], 1, tot_obj_transl.shape[1])
      ...
      sv_predicted_infos_fn_to_statistics[sv_predicted_info_fn] = (APD, smoothness, dist_dist_avg, consistency_value)
  ...
  # sv_predicted_infos_fn_to_statistics
  sv_predicted_infos_fn_to_statistics_fn = "predicted_infos_fn_to_statistics.npy"
  np.save(sv_predicted_infos_fn_to_statistics_fn, sv_predicted_infos_fn_to_statistics)
  print(f"predicted infos fn to statistics saved to {sv_predicted_infos_fn_to_statistics_fn}")
