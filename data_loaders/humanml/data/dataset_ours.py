...
class GRAB_Dataset_V16(torch.utils.data.Dataset):
    ...
    # sigma value of data in all the sequences #
    ...
    def __getitem__(self, index):
        ...
        #     ## rhand_joints * joints_scale_up = (trans_canoncialized_joints * joints_scale_up + trans * joints_scale_up) ## # avg and std positions in the space -> 
        #     rel_base_pts_to_pert_rhand_joints = torch.matmul(
        ...
        ''' Joints mean and std -> strategy 2 '''
        # avg_joints = torch.mean(rt_rhand_joints, dim=0, keepdim=True) # 1 x nnjts x 3 
        # std_joints = torch.std(rt_rhand_joints, dim=0, keepdim=True) # 1 x nnjts x 3
        # if self.inst_normalization:
        #     if self.args.debug:
        #         print(f"normalizing joints using mean: {avg_joints}, std: {std_joints}")
        #     rt_rhand_joints = (rt_rhand_joints - avg_joints) / std_joints
        ''' Joints mean and std -> strategy 2 '''
        ...
        if self.inst_normalization:
            # if self.args.debug:
            #     print(f"normalizing joints using mean: {avg_joints}, std: {std_joints}")
            ### denoising; calcualte correspondin
            # 1) normalize joints; 2) add noise in the normalized space; 3) change to the original space; 4) calculate the base_pts_to_joints features
        ...
        m_length = self.window_size # shakes 
...
class GRAB_Dataset_V17(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.denoising_stra == "rep":
            ...
            ''' Stra 1 -> per frame with joints '''
            # nf x nnj x nnb x 3 # # per_frame_avg, std, #
...
# encoding decodeing;
# transformers for sequential data modeling #
### GRAB_dataset_V15 ###
# GRAB dataset with pose # # GRAB_Dataset_V16 #
...
class GRAB_Dataset_V19(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        obj_pts_disp = base_pts_tot[1:, :, :] - base_pts_tot[:-1, :, :] # (ws - 1) x nn_obj_pts x 3 #
        # rhand_joints_disp: (ws - 1) x nn_hand_pts x 3 ---> (ws - 1) x nn_hand_pts x
        ...
        vel_obj_pts_to_hand_pts = rhand_joints_ncanon_disp.unsqueeze(2) - obj_pts_disp.unsqueeze(1) ## obj_pts_disp # 
        
        # if not self.aug and (not self.args.not_canon_rep):
        #     vel_obj_pts_to_hand_pts = torch.matmul( # # (ws - 1) x nnj x nnb x 3 # > relatie evelocities # 
        #         vel_obj_pts_to_hand_pts, object_global_orient_mtx_th.transpose(1, 2)[:-1].unsqueeze(1) ## (ws - 1) x 1 x 1 x 3 x 
        #     )
        #     # global orientation #
        #     # obj_pts_disp = 

        # obj_to_rhand_joints_rel_vel = torch.matmul( # (ws - 1) x nnj x 3; the relative velocity #
        #     rhand_joints_disp, object_global_orient_mtx_th.transpose(1, 2)[:-1] # relative velociity # 
        # )
