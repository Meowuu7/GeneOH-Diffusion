...
<!-- :exclamation: **A known issue**

We have observed that the object motion pattern generated by executing this repository (as demonstrated in the above example, or by attempting to process a different sequence and reviewing its outcome) differs from our initial demos (available on our website). We suppose that during code cleanup, subtle deviations in the object motion processing procedure of HOI4D may have occurred due to unexpected code deletions or additions. We will rectify this issue after cleaning up and releasing the training process. -->
<!-- We suppose that it is due to a subtle difference in the object motion processing  -->
...
Download [preprocessed data (train split)](https://1drv.ms/u/s!AgSPtac7QUbHgWIvblOocQ43ZVxG?e=8qIjho) and extract it under the folder for preprocessed GRAB data, *e.g.,* `data/grab/GRAB_processed`. 

Download [preprocessed data (hand, train split)](https://1drv.ms/u/s!AgSPtac7QUbHgWOY8ZdLgApKHd_j?e=dL6z2e) and extract it under the folder for GRAB preprocessed subject data (e.g., `./data/grab/GRAB_processed_wsubj`). 
