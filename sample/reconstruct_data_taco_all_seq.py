...
def get_optimized_hand_fr_joints_v4_anchors(joints, base_pts, tot_base_pts_trans, tot_base_normals_trans, with_contact_opt=False, nn_hand_params=24, rt_vars=False, with_proj=False, obj_verts_trans=None, obj_faces=None, with_params_smoothing=False, dist_thres=0.005, with_ctx_mask=False):
  ...
  for i in range(num_iters): #
      ...
      rot_diff = rot_var.view(nn_frames, -1)[1:] - rot_var.view(nn_frames, -1)[:-1]
      rot_smoothness_loss = F.mse_loss(rot_diff[:, 1:], rot_diff[:, :-1])
      ...
      # loss = joints_pred_loss * 30
      loss = joints_pred_loss * 1000 + rot_smoothness_loss * 10.0
      ...
      print('\tPose Prior Loss: {}'.format(pose_prior_loss.item()))
      print('\tPose Smoothness Loss: {}'.format(pose_smoothness_loss.item()))
      print('\tJoints Prediction Loss: {}'.format(rot_smoothness_loss.item()))
  ...
  for i in range(num_iters):
      ...
      # loss = joints_pred_loss * 5000 + pose_smoothness_loss * 0.000001 + shape_prior_loss * 0.0002 + pose_prior_loss * 0.0005 # + attaction_loss * 10000 # + joints_smoothness_loss * 200.
      loss = joints_pred_loss * 5000 + pose_smoothness_loss * 0.00000000001 + shape_prior_loss * 0.0002 + pose_prior_loss * 0.0005 # + attaction_loss * 10000 # + joints_smoothness_loss * 200.
      
      # loss = joints_pred_loss * 5000 + pose_smoothness_loss * 0.03 + shape_prior_loss * 0.001 + pose_prior_loss * 0.08 # + attaction_loss * 10000 # + joints_smoothness_loss * 200.
      ...
      #   loss = joints_pred_loss * 5000 + pose_smoothness_loss * 0.03+ shape_prior_loss * 0.001 + pose_prior_loss * 0.1 # + attaction_loss * 10000 # + joints_smoothness_loss * 200.
...
def get_proj_losses(hand_verts, base_pts, base_normals, tot_penetration_masks, nearest_base_pts=None, nearest_base_normals=None):
  ...
  # if there are no object normals #
  tot_proj_loss_masks = tot_penetration_masks.float() > 0.5
...
if __name__=='__main__':
  ...
  pred_infos_sv_folder = f"/data2/sim/eval_save/HOI_Arti/{cat_nm}"
  pred_infos_sv_folder = f"/data1/xueyi/mdm/eval_save/"
  pred_infos_sv_folder = f"/data2/sim/eval_save/GRAB"
  
  pred_infos_sv_folder = "/data/datasets/genn/sim/arctic_save_res"
  new_pred_infos_sv_folder = "/data2/datasets/sim/arctic_save_res"
  ...
  pred_infos_sv_folder = "/data2/datasets/sim/arctic_save_res"
  new_pred_infos_sv_folder = "/data2/datasets/sim/arctic_save_res"
...
#   use_t = 400
  ...
  # test_tag = "jts_t_50_rep_arctic_use_left_st_100_"
  # test_tags = [f"jts_t_50_rep_arctic_use_left_st_100_", "jts_t_50_rep_arctic_use_left_st_130_", "jts_t_50_rep_arctic_use_left_st_160_", "jts_t_50_rep_arctic_use_left_st_190_", "jts_t_50_rep_arctic_use_left_st_220_", "jts_t_50_rep_arctic_use_left_st_250_"]
    
  
  ### test tag and test tags ###
  # jts_t_200_rep_arctic_use_left_st_100_posepred_wjtspred_
  # test_tag = "jts_t_200_rep_arctic_use_left_st_100_posepred_wjtspred_"
  # test_tags = [f"jts_t_200_rep_arctic_use_left_st_100_posepred_wjtspred_"]
  
#   test_tags = ['jts_spatial_t_200_hho_', 'jts_spatial_t_200_st_50_hho_', 'jts_spatial_t_200_st_80_hho_']
  
#   test_tags = ['20231104_001_jts_t_400_st_20_hho_']
#   test_tags = ['20231104_001_jts_t_400_st_20_hho_']
  
#   test_tags = ['20231104_001_jts_t_200_st_20_hho_']
  
#   single_seq_path = "/data3/hlyang/results/test_data/20231104/20231104_001.pkl"
  
  
  
  folder = "/data3/hlyang/results/test_data/20231104"
  tot_seqs = os.listdir(folder)
  tot_seqs = [seq_nm for seq_nm in tot_seqs if seq_nm.endswith(".pkl")]
  
  for cur_seq_nm in tot_seqs:
    pure_nm = cur_seq_nm.split(".")[0]
    
    single_seq_path = os.path.join(folder, cur_seq_nm)
    test_tag = f"{pure_nm}_jts_spatial_t_{100}_"
        
    
    data_dict = pkl.load(open(single_seq_path, 'rb'))
    data_hand_verts = data_dict['hand_verts']
    nn_frames = data_hand_verts.shape[0] 
    nn_st_skip = 30
    num_cleaning_frames = 60 # args.num_frames
    num_ending_clearning_frames = nn_frames - num_cleaning_frames + 1 ## nn_st_skip ##
    # nn_end_st = nn_frames - 
    ...
    # test_tags = [f"20231104_001_jts_t_400_hho__st_{cur_st}" for cur_st in st_idxes]
    # test_tags = [f"20231104_001_jts_t_100_hho__st_{cur_st}" for cur_st in st_idxes]
    # # test_tags = [f"20231104_001_jts_t_50_hho__st_{cur_st}" for cur_st in st_idxes]
    # test_tags = [f"20231105_001_jts_spatial_t_100_hho__st_{cur_st}" for cur_st in st_idxes]
    test_tags = [f"{pure_nm}_jts_spatial_t_100__st_{cur_st}" for cur_st in st_idxes]
    
    # test_tags = [f"20231104_001_jts_spatial_t_50_hho__st_{cur_st}" for cur_st in st_idxes]
    # test_tags = [f"20231104_001_jts_spatial_t_100_hho__st_{cur_st}" for cur_st in st_idxes]
    ...
    ed_idx = 80
    # pred_infos_sv_folder = "/data2/datasets/sim/arctic_save_res"
    # new_pred_infos_sv_folder = "/data2/datasets/sim/arctic_save_res" # wmaskanchors_multi_ntag_6
    
    ## pred infos sv folder ##
    pred_infos_sv_folder = "/data3/datasets/xueyi/hho_save_res/"
    new_pred_infos_sv_folder = "/data3/datasets/xueyi/hho_save_res/"
    ...
    # tot_rnd_seeds = range(11, 121, 11)
    # tot_rnd_seeds = [55]
    # tot_rnd_seeds = range(77, 88, 11)
    # tot_rnd_seeds = range(33, 121, 11)
    ### Articulated ####
    
    # predicted_infos_seq_0_test_seed_22_tag_jts_spatial_t_200_hho_.npy
    tot_seq_paths = [f"predicted_infos_seq_0_test_seed_{cur_seed}_tag_jts_spatial_t_200_hho_.npy" for cur_seed in tot_rnd_seeds]
    tot_seq_paths = [os.path.join(pred_infos_sv_folder, cur_seq_path) for cur_seq_path in tot_seq_paths]
    tot_seq_tags = [f"{cur_seed}_jts_spatial_t_200" for cur_seed in tot_rnd_seeds]
    
    tot_seq_paths = tot_seq_paths[:1]
    tot_seq_tags = tot_seq_tags[:1]

    # tot_arctic_seq_paths, tot_arctic_seq_tags = get_arctic_seq_paths()
    
    # tot_arctic_seq_paths = tot_arctic_seq_paths[19:]
    # tot_arctic_seq_tags = tot_arctic_seq_tags[19:]
    st_idxx = 0
    # st_idxx = 19
    ...
    for i_test_seq, (cur_seq_path, cur_seq_tag) in enumerate(zip(tot_seq_paths, tot_seq_tags)):
        ...
        for seed in tot_rnd_seeds:
            ...
            merged_pred_sv_infos_sv_fn_nm = f"predicted_infos_sv_dict_seed_{seed}_tag_{test_tags[0]}_{cur_seq_tag}_multi_ntag_{len(test_tags)}.npy"
            ...
            if len(outputs.shape) == 2:
                ...
                if len(tot_base_pts.shape) == 2:
                    ...
                    # numpy array # optimized sv infos #
            ...
            #### obj_verts_trans, obj_faces ####
            ## obj_verts, tot_obj_rot ## #### verts, tot_obj_rot, tot_obj_transl ####
