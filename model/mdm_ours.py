...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        self.input_feats = 21 * (3 + 3 + 3 + 1)
        self.input_process = InputProcessObjBase(self.data_rep, self.input_feats+self.gru_emb_dim, self.latent_dim)
        ...
        if self.cond_mode != 'no_cond': # cond_mode != 'no_cond'
        ...
        self.output_process = OutputProcessObjBase(self.data_rep, self.latent_dim)
    ...
    def forward(self, x, timesteps):
        ...
        # print(f"cond_mode: {self.cond_mode}")
        force_mask = x.get('uncond', False)
        ## ==== encode TEXT ==== ##
        if 'text' in self.cond_mode:
            enc_text = self.encode_text(x['text'])
            emb += self.embed_text(self.mask_cond(enc_text, force_mask=force_mask))
        ## ==== encode ACTION ==== ##
        if 'action' in self.cond_mode:
            action_emb = self.embed_action(x['action'])
            emb += self.mask_cond(action_emb, force_mask=force_mask)

        # if self.arch == 'gru':
        #     ## ==== RESHAPE x to bsz x nn_feats x 1  x nn_frames ==== ##
        #     x_reshaped = x.reshape(bs, njoints*nfeats, 1, nframes)
        #     ## ==== gru embeddings -> n_frames ==== ##
        #     emb_gru = emb.repeat(nframes, 1, 1)     #[#frames, bs, d]
        #     emb_gru = emb_gru.permute(1, 2, 0)      #[bs, d, #frames]
        #     emb_gru = emb_gru.reshape(bs, self.latent_dim, 1, nframes)  #[bs, d, 1, #frames]
        #     ## === additional embedding features and gru embedding inputs features === ##
        #     x = torch.cat((x_reshaped, emb_gru), axis=1)  #[bs, d+joints*feat, 1, #frames]

        # x = self.input_process(x)
        
        obj_base_feats = obj_base_feats.permute(0, 1, 3, 2, 4).contiguous().view(bsz, nframes, nnb, -1).contiguous()
        
        obj_base_feats = self.input_process(obj_base_feats)

        if self.arch == 'trans_enc':
            # adding the timestep embed
            xseq = torch.cat((emb, obj_base_feats), axis=0)  # [seqlen+1, bs, d]
            xseq = self.sequence_pos_encoder(xseq)  # [seqlen+1, bs, d]
            output = self.seqTransEncoder(xseq)[1:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]

        elif self.arch == 'trans_dec':
            if self.emb_trans_dec:
                xseq = torch.cat((emb, obj_base_feats), axis=0)
            else: # xseq --> 
                xseq = obj_base_feats
            xseq = self.sequence_pos_encoder(xseq)  # [seqlen+1, bs, d]
            if self.emb_trans_dec:
        ...
        output = self.output_process(output, x)  # [bs, njoints, nfeats, nframes]
        return output
...
class MDMV2(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        # self.input_feats = 21 * (3 + 3 + 3 + 1) 
        # ## [base_pts, nearest_obj_pts, nearest_obj_vns, relative joint position, relative joint distances] ##
        self.input_feats = 21 * (3 + 3 + 3 + 3 + 1) #
        # input feats dim + gru emb dim #
        self.input_process = InputProcessObjBase(self.data_rep, self.input_feats+self.gru_emb_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps):
        ...
        obj_base_feats = torch.cat(
          [base_pts_exp, sampled_base_pts_nearest_obj_pc_exp, sampled_base_pts_nearest_obj_vns_exp, rel_base_pts_to_rhand_joints, dist_base_pts_to_rhand_joints.unsqueeze(-1)], dim=-1
        )
        # obj_base_feats = self.
...
class MDMV3(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        # self.input_feats = 21 * (3 + 3 + 3 + 3 + 1) #
        self.input_feats = 21 * (3 + 3 + 3 + 1) #
        # input feats dim + gru emb dim # # InputProcessObjBase: pointnetpp for encoding ##
        self.input_process = InputProcessObjBase(self.data_rep, self.input_feats+self.gru_emb_dim, self.latent_dim)
        ...
        if self.arch == 'trans_enc':
            ...
            raise ValueError('Please choose correct architecture [trans_enc, trans_dec, gru]')
        # tiemstep # # timestep embedding e #
        self.embed_timestep_e = TimestepEmbedder(self.latent_dim, self.sequence_pos_encoder)
    ...
    def forward(self, x, timesteps):
        ...
        # bsz x ws - 1 x nnb x 3
        # print(f"e_disp_rel_to_base_along_normals: {e_disp_rel_to_base_along_normals.size()}, e_disp_rel_to_baes_vt_normals: {e_disp_rel_to_baes_vt_normals.size()}")
        # e_obj_base_feats: bsz x ws x nnj x nnb x (xxx)
        e_obj_base_feats = torch.cat(
            [base_pts_exp[:, :-1, :, :], base_normals_exp[:, :-1, :, :], rel_base_pts_to_rhand_joints[:, :-1, :, :, :], e_disp_rel_to_base_along_normals.unsqueeze(-1), e_disp_rel_to_baes_vt_normals.unsqueeze(-1)], dim=-1
        )
        # obj_base_feats = self.
        ...
        # bs, njoints, nfeats, nframes = x.shape
        e_emb = self.embed_timestep(timesteps)  # [1, bs, d]
        ...
        e_obj_base_feats = e_obj_base_feats.permute(0, 1, 3, 2, 4).contiguous().view(bsz, nframes - 1, nnb, -1).contiguous()
        
        e_obj_base_feats = self.input_process_e(e_obj_base_feats)
        ...
        if self.arch == 'trans_enc':
            ...
            # adding the timestep embed
            e_xseq = torch.cat((e_emb, e_obj_base_feats), axis=0)  # [seqlen+1, bs, d]
            e_xseq = self.sequence_pos_encoder_e(e_xseq)  # [seqlen+1, bs, d]
            e_output = self.seqTransEncoder_e(e_xseq)[1:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
        elif self.arch == 'trans_dec':
            if self.emb_trans_dec:
                xseq = torch.cat((emb, obj_base_feats), axis=0)
            else: # xseq --> 
            ...
            if self.emb_trans_dec:
                e_xseq = torch.cat((e_emb, e_obj_base_feats), axis=0)
            else: # xseq --> 
                e_xseq = e_obj_base_feats
            e_xseq = self.sequence_pos_encoder_e(e_xseq)  # [seqlen+1, bs, d]
            if self.emb_trans_dec:
                e_output = self.seqTransDecoder_e(tgt=e_xseq, memory=e_emb)[1:] # [seqlen, bs, d] # FIXME - maybe add a causal mask
            else:
                e_output = self.seqTransDecoder_e(tgt=e_xseq, memory=e_emb)
        elif self.arch == 'gru':
            xseq = obj_base_feats
            xseq = self.sequence_pos_encoder(xseq)  # [seqlen, bs, d]
            output, _ = self.gru(xseq)
            ...
            e_xseq = e_obj_base_feats
            e_xseq = self.sequence_pos_encoder_e(e_xseq)  # [seqlen, bs, d]
            e_output, _ = self.gru_e(e_xseq)

        output = self.output_process(output, x)  # [bs, njoints, nfeats, nframes]
        e_output = self.output_process_e(e_output, x)
...
class MDMV4(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        self.joints_feats_in_dim = 21 * 3 # -> joints feats in dim #
    ...
    def forward(self, x, timesteps):
        ...
        if self.arch == 'trans_enc':
            xseq = torch.cat((rhand_joints_emb, rhand_joints_feats), axis=0)  # [seqlen+1, bs, d]
            xseq = self.joint_sequence_pos_encoder(xseq)  # [seqlen+1, bs, d]
            joint_seq_output = self.joint_sequence_seqTransEncoder(xseq)[1:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
        elif self.arch == 'trans_dec':
            if self.emb_trans_dec:
                xseq = torch.cat((rhand_joints_emb, rhand_joints_feats), axis=0)
            else: # xseq --> 
                xseq = rhand_joints_feats
            xseq = self.joint_sequence_pos_encoder(xseq)  # [seqlen+1, bs, d]
            if self.emb_trans_dec:
                joint_seq_output = self.joint_sequence_seqTransDecoder(tgt=xseq, memory=rhand_joints_emb)[1:] # [seqlen, bs, d] # FIXME - maybe add a causal mask
            else:
                joint_seq_output = self.joint_sequence_seqTransDecoder(tgt=xseq, memory=rhand_joints_emb)
        elif self.arch == 'gru':
            xseq = rhand_joints_feats
            xseq = self.joint_sequence_pos_encoder(xseq)  # [seqlen, bs, d]
            joint_seq_output, _ = self.joint_sequence_gru(xseq)
        # bsz x ws x nnj x 3 #
        joint_seq_output = self.joint_sequence_output_process(joint_seq_output, pert_rhand_joints)  # [bs, njoints, nfeats, nframes]
        # bsz x ws x nnj x nnfeats # --> joints_seq_outputs #
        joint_seq_output = joint_seq_output.permute(0, 3, 1, 2).contiguous()
            
        ## bsz x nframes x nnj x nnb x 3 ##
        bsz, nframes, nnj, nnb = x['rel_base_pts_to_rhand_joints'].shape[:4]
        # forward the samole 
        base_pts = x['base_pts'] # bsz x nnb x 3
        base_normals = x['base_normals'] # bsz x nnb x 3
        ...
        # bs, njoints, nfeats, nframes = x.shape
        e_emb = self.embed_timestep_e(timesteps)  # [1, bs, d]
        ...
        ## 
        return joint_seq_output, output, e_output
...
class MDMV5(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        # self.input_feats = 21 * (3 + 3 + 3 + 3 + 1) #
        ### nn_input_feats ##
        # self.input_feats = 21 * (3 + 3 + 3 + 1) # input_feats #
        # input feats dim + gru emb dim # # InputProcessObjBase: pointnetpp for encoding ##
        ...
        self.rel_input_feats = 21 * 3 # relative positions from base pts to rhand joints ##
        ...
        if self.arch == 'trans_enc':
            print("TRANS_ENC init") # encode noisy inputs to the latent space #
            seqTransEncoderLayer = nn.TransformerEncoderLayer(d_model=self.latent_dim,
                                                              nhead=self.num_heads,
                                                              dim_feedforward=self.ff_size,
                                                              dropout=self.dropout,
            ...
            self.seqTransEncoder = nn.TransformerEncoder(seqTransEncoderLayer,
                                                         num_layers=self.num_layers)
        elif self.arch == 'trans_dec': # denoiser #
            print("TRANS_DEC init")
            seqTransDecoderLayer = nn.TransformerDecoderLayer(d_model=self.latent_dim,
                                                              nhead=self.num_heads,
                                                              dim_feedforward=self.ff_size,
        ...
        basejtsrel_seqTransEncoderLayer = nn.TransformerEncoderLayer(d_model=self.latent_dim,
                                                            nhead=self.num_heads,
                                                            dim_feedforward=self.ff_size,
                                                            dropout=self.dropout,
                                                            activation=self.activation)

        self.basejtsrel_denoising_seqTransEncoder = nn.TransformerEncoder(basejtsrel_seqTransEncoderLayer,
                                                        num_layers=self.num_layers)
        
        # OutputProcessObjBaseRaw ## output process for basejtsrel #
        self.output_process = OutputProcessObjBaseRaw(self.data_rep, self.latent_dim)
        ##### ==== input process, communications, output process for rel, dists ==== #####
    ...
    def dec_latents_to_joints_with_t(self, input_latent_feats, x, timesteps):
        # joints_seq_latents: seq x bs x d --> perturbed joints_seq_latents \in [-1, 1] ##
        ...
        # print(f"base_jts_e_feats: {base_jts_e_feats.size()}, rel_base_pts_outputs: {rel_base_pts_outputs.size()}, joints_seq_latents: {joints_seq_latents.size()}")
        ...
        joints_seq_time_emb = self.joint_sequence_denoising_embed_timestep(timesteps)
        ...
        basejtsrel_output = self.output_process(basejtsrel_seq_latents, x) # bsz x nframes x nnb x nnj x 3 ## --> for the decoded rel positions
    ...
    def forward(self, x, timesteps):
        ...
        base_jts_e_feats = self.seqTransEncoder_e(base_jts_e_feats)
        ...
        nnb = rel_base_pts_to_rhand_joints.size(3)
        ...
        rel_base_pts_feats = self.input_process(rel_base_pts_to_rhand_joints_exp)
        rel_base_pts_feats_pos_embedding = self.sequence_pos_encoder(rel_base_pts_feats)
        rel_base_pts_outputs = self.basejtsrel_denoising_seqTransEncoder(rel_base_pts_feats_pos_embedding)
        ...
        # joint_seq_output: seqlen x bs x d # 
        # add noise in this step -> [-1, 1] value space #
        # joint_seq_output : seqlen x bs x d #
        # joint_seq_output = (torch.sigmoid(joint_seq_output) - 0.5) * 2. # scale to [-1, 1]
...
class MDMV6(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        if self.diff_jts:
            ...
            # joint_sequence_input_process, joint_sequence_pos_encoder, joint_sequence_seqTransEncoder # 
            self.joint_sequence_input_process = InputProcess(self.data_rep, self.joints_feats_in_dim, self.latent_dim)
            # # InputProcessObjBase(self, data_rep, input_feats, latent_dim)
            # self.joint_sequence_input_process = InputProcessObjBase(self.data_rep, 3, self.latent_dim)
            # self.input_process = InputProcess(self.data_rep, self.input_feats+self.gru_emb_dim, self.latent_dim)
            ...
            seqTransEncoderLayer = nn.TransformerEncoderLayer(d_model=self.latent_dim,
                                                            ...
                                                            dropout=self.dropout,
                                                            activation=self.activation)
            ## Joint sequence transformer encoder layer ##
            self.joint_sequence_seqTransEncoder = nn.TransformerEncoder(seqTransEncoderLayer,
                                                        num_layers=self.num_layers)
            # elif self.arch == 'trans_dec':
            #     print("TRANS_DEC init")
    ...
    def set_enc_to_eval(self):
        ...
        # input_process_e, sequence_pos_encoder_e, seqTransEncoder_e #
        if self.diff_basejtse:
            self.input_process_e.eval()
            self.sequence_pos_encoder_e.eval()
            self.seqTransEncoder_e.eval()
        # input_process, sequence_pos_encoder, basejtsrel_denoising_seqTransEncoder #
        if self.diff_basejtsrel:
    ...
    def dec_latents_to_joints_with_t(self, input_latent_feats, x, timesteps):
        # joints_seq_latents: seq x bs x d --> perturbed joitns_seq_latents \in [-1, 1] ##
        # def dec_latents_to_joints_with_t(self, joints_seq_latents, timesteps):
        ## positional encoding for denoising ## ## denoising ##
        # rt_dict = {
            # 'joint_seq_output': joint_seq_output,
            # 'rel_base_pts_outputs': rel_base_pts_outputs, ## rel bae pts outputs ## 
        # }
        # ## rt_dict --> ## #
        rt_dict = {}
        if self.diff_jts:
            ...
            if not self.args.without_dec_pos_emb: ## not with the pose encoder ##
                print("With dec pos embedding!")
            ...
            ### for the denoised latents? ###
            
            # joints_seq_latents: seq_len x bs x d
            # joints_seq_latents_exp = joints_seq_latents.view(joints_seq_latents.size(0)* joints_seq_latents.size(1), -1).contiguous()
            # minn_joints_seq_latents, _ = torch.min(joints_seq_latents_exp, dim=0)
            # maxx_joints_seq_latents, _ = torch.max(joints_seq_latents_exp, dim=0)
            # # print(f"minn_latents_for_decoding: {minn_joints_seq_latents[:10]}, maxx_latents_for_decoding: {maxx_joints_seq_latents[:10]}")
        ...
        if self.diff_basejtsrel:
            ...
            basejtsrel_output = self.output_process(basejtsrel_seq_latents, x)
    ...
    def forward(self, x, timesteps):
        ...
        if self.diff_basejtse:
            ...
            ### Embed physicss quantities ###
            if self.use_sigmoid:
                base_jts_e_feats = torch.sigmoid(base_jts_e_feats) - 0.5
                base_jts_e_feats = base_jts_e_feats * 2.
        ...
        if self.diff_jts:
            ...
            joint_seq_output = self.joint_sequence_seqTransEncoder(xseq) # [1:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
            if self.use_sigmoid:
                joint_seq_output = torch.sigmoid(joint_seq_output) - 0.5
                joint_seq_output = joint_seq_output * 2.
            rt_dict['joint_seq_output'] = joint_seq_output
            
        if self.diff_basejtsrel:
            # input_process, sequence_pos_encoder, seqTransEncoder #
            ### === Encode input joint sequences === ###
            # base_normals = x['base_normals'] # bsz x nnb x 3
            # bsz x ws x nnj x nnb x 3 #
            rel_base_pts_to_rhand_joints = x['rel_base_pts_to_rhand_joints'] # bsz x ws x nnj x nnb x 3 # bsz x ws x nnj x nnb x 3 #
            ...
            rel_base_pts_outputs = self.seqTransEncoder(rel_base_pts_feats_pos_embedding)
            if self.use_sigmoid:
                rel_base_pts_outputs = torch.sigmoid(rel_base_pts_outputs) - 0.5
                rel_base_pts_outputs = rel_base_pts_outputs * 2.
...
class MDMV7(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        if self.diff_basejtsrel:
            ...
            if self.args.use_dec_rel_v2:
                self.output_process = OutputProcessObjBaseRawV2(self.data_rep, self.latent_dim)
            else:
                # OutputProcessObjBaseRaw ## output process for basejtsrel #
                self.output_process = OutputProcessObjBaseRaw(self.data_rep, self.latent_dim)
                ##### ==== input process, communications, output process for rel, dists ==== #####
    ...
    def forward(self, x, timesteps):
        ...
        if self.diff_basejtsrel:
            ...
            # seqTransEncoder, logvar_seqTransEncoder
            rel_base_pts_outputs_mean = self.seqTransEncoder(rel_base_pts_feats_pos_embedding)
            ### calculate logvar, mean, and feats ###
            rel_base_pts_outputs_logvar = self.logvar_seqTransEncoder(rel_base_pts_feats_pos_embedding)
