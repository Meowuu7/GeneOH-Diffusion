...
def merge_verts_faces(verts, faces):
  tot_verts_nn = 0
  tot_merged_faces = []
  for i_obj in range(len(verts)):
    cur_obj_verts_nn = verts[i_obj].shape[0]
    if isinstance(faces[i_obj], list):
      cur_faces = np.array(faces[i_obj], dtype=np.long)
    else:
      cur_faces = faces[i_obj]
    cur_faces = cur_faces + tot_verts_nn
    tot_verts_nn += cur_obj_verts_nn
  tot_merged_faces = np.concatenate(tot_merged_faces, axis=0)
  tot_merged_verts = np.concatenate(verts, axis=0)
  return tot_merged_verts, tot_merged_faces
...
def save_transformed_objs(obj_verts, obj_faces, obj_pose_folder):
  ...
  transformed_obj_verts, transformed_obj_faces = [], []
  for cur_pkl_fn in tot_pkl_fns:
    print(f"processing {cur_pkl_fn}")
    ...
    for i_part in range(len(obj_verts)):
      ...
      transformed_obj_verts.append(transformed_verts)
      transformed_obj_faces.append(cur_part_obj_faces)
  return transformed_obj_verts, transformed_obj_faces
...
def read_hand_and_objects(hand_obj_fn, cad_model_fn, obj_pose_folder):
  obj_verts = []
  obj_faces = []
  for i_part in range(len(cad_model_fn)):
    ...
    obj_verts.append(cur_part_obj_verts)
    obj_faces.append(cur_part_obj_faces)
  trans_verts, trans_faces = save_transformed_objs(obj_verts, obj_faces, obj_pose_folder)
  tot_merged_verts, tot_merged_faces = merge_verts_faces(trans_verts, trans_faces)
  
  ##### load hand obj based on mano #####
  manolayer = ManoLayer(
          mano_root='mano/models', use_pca=False, ncomps=45, flat_hand_mean=True, side='right')
  f = open(hand_obj_fn, 'rb')
  hand_info = pickle.load(f, encoding='latin1')
  f.close()
